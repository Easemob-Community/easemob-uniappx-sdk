# 环信 IM SDK UniAppX 工程版 — 用户接入与测试指南
...
> **⚠️ 免责声明（必读）**
>
> 本内容为开源集成环信 SDK UniAppX 的示例，不排除存在边界外问题。请谨慎使用于生产环境，如果出现问题后果属于预期、。
...
- 开发环境需能访问 Maven Central（Android）和 CocoaPods 官方源（iOS）。
- 如在中国大陆且 CocoaPods 下载慢，建议配置国内镜像或提前缓存 `HyphenateChat` 4.20.0 版本。
...
将本工程中的如下目录，**整体复制**到你的 UniAppX 项目根目录下：

```
uts-sdk-demo/uni_modules/easemob-uts-sdk-beta/
    ├── package.json
    ├── readme.md
    └── utssdk/
        ├── index.uts              ← 统一入口
...
**Android 权限说明**：
SDK 模块内部已包含必需的 `AndroidManifest.xml` 权限（INTERNET、ACCESS_NETWORK_STATE 等），通常无需手动添加。
...
本工程 `uts-sdk-demo/pages/` 下提供 5 个测试页面：
...
3. 模拟 HBuilderX 的调用方式验证：
   ...
   ```
4. 若上述命令报错，请修复本地 Ruby 环境（参考本文档同级目录 `hbuilderx-pod-install-troubleshooting.md`）。

### Q3: Android 编译报错 "找不到名称 'setMessageStatusCallback'"

本 SDK 已通过 Kotlin 辅助类 `MessageHelper.kt` 解决此问题。如在自己的项目中遇到类似问题，请确保：
- Kotlin 文件位于 `utssdk/app-android/` 目录下
- 包名格式为 `uts.sdk.modules.{模块名}`
- 方法使用 `@JvmStatic` 注解
...
| 文档 | 说明 |
|------|------|
| `development-guide.md` | 内部开发指南（架构设计、桥接层说明、踩坑记录） |
| `uni-app-x-common-issues.md` | UTS 编译常见问题与解决方案 |
| `hbuilderx-pod-install-troubleshooting.md` | iOS CocoaPods 排错完整指南 |
| `version-update-4.15.1.md` | 环信 SDK 版本升级记录（参考） |
| `uts-module-export-guide.md` | UTS 模块导出机制说明 |
